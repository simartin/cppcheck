...
void Variable::evaluate(const Library* lib)
{
    ...
    for (const Token* const end = _name?_name:_end; tok != end;) {
        if (tok->str() == "static")
            ...
            if (isArray()) {
                const bool pointerToArray = Token::Match(tok->previous(), "( * %name% ) [");
                setFlag(fIsPointer, pointerToArray);
                setFlag(fIsArray, !pointerToArray);
                if (pointerToArray) _dimensions.clear();
            } else {
